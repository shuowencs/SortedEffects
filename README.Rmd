...
The economic question is: holding $W$ fixed, how do changes in $T$ affect $Y$? The mathematical expression for partial effects is  $$\Delta(x) = g(t_{1}, w)-g(t_{0}, w)$$
...
It is obvious that $\Delta(x)$ depends on values of $W$. Since sample observations will have different values of $W$, $\Delta(x)$ features heterogeneity. Because $\Delta(x)$ is unobservable in data, we use econometric techniques to get an estimate. However, no estimation is accurate. Therefore we need to quantify the uncertainty of estimation, and in econometrics this is called **inference**.  
